...
export async function ensureHealthMonitor() {
  ...
  async function getLock() {
    try { const id = String(process.pid); const ok = await redis.set("sys:db:lock", id, { NX: true, EX: 30 }); return !!ok; } catch { return false; }
  }
  async function renew() { try { const id = String(process.pid); await redis.set("sys:db:lock", id, { EX: 30 }); } catch {} }
  async function pingDb() {
    try { const db = await getMysql(); await db.execute("SELECT 1", []); return true; } catch { return false; }
  }
  async function loop() {
    try {
      ...
      for (;;) {
        ...
        if (ok) {
          await setSnapshot({ status: "up", backoffMs: 1000 });
          await publish(redis, { type: "status", status: "up", at: Date.now(), backoffMs: 1000 });
          await renew();
          await new Promise((r) => setTimeout(r, 2000));
        } else {
          backoff = Math.min(backoff * 2, 120000);
          await setSnapshot({ status: "down", backoffMs: backoff });
          await publish(redis, { type: "status", status: "down", at: Date.now(), backoffMs: backoff });
          await renew();
